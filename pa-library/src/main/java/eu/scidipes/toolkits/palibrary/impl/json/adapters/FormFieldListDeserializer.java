...
/**
 * Deserialiser for JSON arrays of {@link FormField} objects, such that the correct implementation type is used.
 * 
 * 
 * @author Simon Berriman
 */
public class FormFieldListDeserializer implements JsonDeserializer<List<FormField>> {
	 ...
	 */
	@Override
	public List<FormField> deserialize(final JsonElement json, final Type typeOfT,
		...
		final FormField[] fa = new FormField[ja.size()];
		...
		for (final JsonElement je : ja) {
			...
			fa[idx++] = context.deserialize(je, FormFieldImpl.class);
		}
